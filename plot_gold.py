...
# -----------------------------
# Step 1: Download Gold via Ticker().history()
# -----------------------------
ticker = "GC=F"
gold_ticker = yf.Ticker(ticker)
data = gold_ticker.history(start="2023-01-01", end="2025-01-01")
...
if data.empty:
    ...
    if 'Close' not in data.columns:
        raise ValueError("No Close column found for Gold!")
    ...
    plt.title('Gold (GC=F) Price & Moving Averages')
    plt.xlabel('Date')
    ...
    plt.title('Gold (GC=F) Daily % Change')
    plt.xlabel('Date')
    plt.ylabel('%')
    plt.legend()
    plt.show()
